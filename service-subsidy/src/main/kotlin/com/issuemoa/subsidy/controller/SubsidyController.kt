...
class SubsidyController(private val subsidyService: SubsidyService) {
    ...
    @GetMapping("/subsidy/{offset}/{limit}")
    fun findSubsidyByWhere(
        ...
        @Parameter(description = "[아동|청년|임산부|노인]") @RequestParam(required = false, defaultValue = "") eligibleRecipients: List<String>,
        @Parameter(description = "[주거·자립|고용·창업]") @RequestParam(required = false, defaultValue = "") serviceCategoryList: List<String>,
        @Parameter(description = "[현금|이용권]") @RequestParam(required = false, defaultValue = "") supportType: List<String>
